...
export const updateEnquiry = async (req, res, next) => {
    try {
        ...
        if (data.estimatedValue !== undefined)
            data.estimatedValue = parseFloat(data.estimatedValue);
...
export const getPublicEnquiry = async (req, res, next) => {
    // ... (rest of the file)
    try {
        const { id } = req.params;
        const enquiry = await prisma.enquiry.findUnique({
...
export const submitDiscoveryForm = async (req, res, next) => {
    try {
        const { id } = req.params;
...
export const extractDiscovery = async (req, res, next) => {
    try {
        const { id } = req.params;
        const file = req.file;
        if (!file) {
        ...
        await prisma.enquiryNote.create({
            data: {
                ...
                userId: req.user?.userId || '' // Assuming auth middleware attaches user
            }
        });
        res.json({
...
export const signProposal = async (req, res, next) => {
    try {
        const { proposalId } = req.params;
...
export const generateDraftEmail = async (req, res, next) => {
    try {
        const { id } = req.params;
        const enquiry = await prisma.enquiry.findUnique({
            where: { id }
        ...
        const draft = await generateColdEmailFlow({
            ...
            senderName: req.user?.displayName || req.user?.name || 'Managing Director'
        });
        res.json(draft);
    }
    catch (error) {
