...
export const finalizeProject = async (req, res) => {
    ...
    try {
        ...
        if (project.status === 'COMPLETED' || project.isLocked)
            return res.status(400).json({ error: 'Project is already finalized or completed' });
...
export const logAdvancePayout = async (req, res) => {
    const { projectId } = req.params;
...
export const updateAdvancePayout = async (req, res) => {
...
export const deleteAdvancePayout = async (req, res) => {
    const { projectId, advanceId } = req.params;
    try {
        const project = await prisma.project.findUnique({ where: { id: projectId } });
        if (!project || project.status === 'COMPLETED' || project.isLocked) {
...
export const getProjectPredictions = async (req, res) => {
    const { projectId } = req.params;
    if (!projectId)
        return res.status(400).json({ error: 'Project ID required' });
    try {
        const predictions = await FinancialService.calculatePredictions(projectId);
        if (!predictions) {
