...
export const checkExistingLeads = async (req, res, next) => {
    try {
        ...
        const results = websites.map(url => {
            const lead = existingLeads.find((l) => l.website === url);
            const inEnquiry = enquiryWebsites.includes(url);
            if (inEnquiry)
                return { website: url, status: 'CONVERTED', exists: true };
            if (lead)
                return { website: url, status: lead.status, exists: true };
...
export const getLeads = async (req, res, next) => {
    try {
        ...
        const parsedLeads = leads.map((lead) => ({
            ...
            suggestedServices: (() => {
                try {
                    return JSON.parse(lead.servicesRequested || '[]');
                }
                catch {
                    return [];
                }
...
export const promoteToEnquiry = async (req, res, next) => {
    try {
        const { id } = req.params;
...
export const importLead = async (req, res, next) => {
    try {
        const { name, website, industry, description, suggestedServices, fitScore } = req.body;
        // Generate a unique Enquiry ID
        const date = new Date();
...
export const generateAILeadEmail = async (req, res, next) => {
    try {
        const { leadId, templateId, senderName } = req.body;
        const lead = await prisma.lead.findUnique({ where: { id: leadId } });
        const template = await prisma.emailTemplate.findUnique({ where: { id: templateId } });
        if (!lead || !template)
            throw new AppError('Lead or template not found', 404);
...
export const sendColdEmail = async (req, res, next) => {
    try {
        ...
        await sendEmail(recipient, subject, body.replace(/\n/g, '<br>'));
        // Update lead status
        await prisma.lead.update({
            where: { id: leadId },
            data: {
