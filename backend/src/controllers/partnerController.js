...
export const updatePartner = async (req, res) => {
    ...
    try {
        ...
        if (!currentPartner)
            return res.status(404).json({ error: 'Partner not found' });
        // Update User part
        const userData = { name, displayName, role, accountStatus, twoFactorEnabled };
        if (password) {
...
export const getPartnerWorkload = async (req, res) => {
    try {
        const partners = await prisma.partner.findMany({
        ...
        const workload = partners.map((partner) => {
            const assigned = activeProjects.filter((p) => p.projectLeadId === partner.id ||
