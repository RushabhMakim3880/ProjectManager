...
export const getCompanySummary = async (req: Request, res: Response) => {
    try {
        ...
        const totalProjectValue = projects.reduce((sum: number, p: ProjectItem) => sum + p.totalValue, 0);
        const activeProjects = projects.filter((p: ProjectItem) => p.status === 'ACTIVE').length;
        const completedProjects = projects.filter((p: ProjectItem) => p.status === 'COMPLETED').length;
