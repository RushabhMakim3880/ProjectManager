...
export class FinancialService {
     ...
     */
    static calculateProfitSharing(GPR, partners, totalPartnerCount) {
        // VALIDATION
        if (GPR < 0)
            throw new Error('GPR must be >= 0');
        if (partners.length === 0)
            throw new Error('At least one partner must exist');
     ...
     */
    static async recalculateProjectFinancials(projectId) {
        ...
        const operationalExpenses = project.transactions
            .filter((t) => t.type === 'EXPENSE' && !t.category.toLowerCase().includes('advance') && !t.category.toLowerCase().includes('draw'))
            .reduce((acc, t) => acc + t.amount, 0);
        ...
        const partnersForCalc = project.contributions.map((c) => ({
            id: c.partnerId,
     ...
     */
    static async calculatePredictions(projectId) {
        const project = await prisma.project.findUnique({
            where: { id: projectId },
            include: {
                transactions: true,
     ...
     */
    static calculatePartnerEarnings(performancePool, contributionPercentage) {
