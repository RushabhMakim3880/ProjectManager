...
async function getAllPartnerEmails() {
...
async function getProjectPartnerEmails(projectId) {
...
export async function notifyTaskAssigned(task, project) {
    if (!task.assignedPartner?.user?.email)
        return;
    const email = task.assignedPartner.user.email;
    const assigneeName = task.assignedPartner.user.name;
    await sendEmail(email, `📋 New Task Assigned: ${task.name}`, `
...
export async function notifyTaskReassigned(task, project, previousAssigneeName) {
    if (!task.assignedPartner?.user?.email)
        return;
    const email = task.assignedPartner.user.email;
    const newAssigneeName = task.assignedPartner.user.name;
    await sendEmail(email, `🔄 Task Re-assigned to You: ${task.name}`, `
...
export async function notifyTaskCompleted(task, project, completedByName) {
    const emails = await getProjectPartnerEmails(project.id);
    if (emails.length === 0)
        return;
    await sendEmail(emails, `✅ Task Completed: ${task.name}`, `
...
export async function notifyTaskComment(task, project, commenter, comment) {
    const emails = await getProjectPartnerEmails(project.id);
    if (emails.length === 0)
        return;
    const typeLabel = comment.type === 'REVIEW' ? '⭐ Review' : '💬 Comment';
    await sendEmail(emails, `${typeLabel} on Task: ${task.name}`, `
...
export async function notifyPayoutFinalized(project, payouts) {
    for (const payout of payouts) {
        ...
        if (!partner?.user?.email)
            continue;
        await sendEmail(partner.user.email, `💰 Payout Finalized: ${project.name}`, `
